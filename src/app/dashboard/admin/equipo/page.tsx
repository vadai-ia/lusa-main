...
export default async function EquipoPage() {
  ...
  const [{ data: profiles }, { data: operators }] = await Promise.all([
    ...
    supabase.schema('lusa').from('operators').select('id, user_id, phone, unit, ruta, is_active'),
  ])

  const opByUserId = Object.fromEntries(
    (operators ?? []).map(o => [o.user_id, { id: o.id, phone: o.phone as string | null, unit: o.unit as string | null, ruta: o.ruta as string | null, is_active: o.is_active as boolean ?? true }])
  )
  ...
  const members = (profiles ?? []).map(p => ({
    ...
    ruta:         opByUserId[p.id]?.ruta       ?? null,
